...
export default function MenJacketsPage({ setSelectedProduct, addToCart, formatPrice, currentUser, setCurrentUser}) {
    ...
    fetch('http://127.0.0.1:5000/api/products')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const filtered = data.filter(p => p.gender === 'men' && p.category === 'jackets');
          setProductsList(filtered);
        } else {
          const fallback = fallbackProducts.filter(p => p.gender === 'men' && p.category === 'jackets');

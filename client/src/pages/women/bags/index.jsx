...
export default function WomenBagsPage({ setSelectedProduct, addToCart, formatPrice, currentUser, setCurrentUser }) {
  // Dùng tên state chung là productsList để copy sang file khác không phải đổi tên
    ...
    fetch('http://127.0.0.1:5000/api/products')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          // LỌC ĐÚNG SẢN PHẨM TÚI NỮ
          const filtered = data.filter(p => p.gender === 'women' && p.category === 'bags');
          setProductsList(filtered);
        } else {
          const fallback = fallbackProducts.filter(p => p.gender === 'women' && p.category === 'bags');
    ...
    <div style={{ padding: "60px 40px", marginTop: "70px", minHeight: "100vh" }}>
        ...
        setCurrentUser={setCurrentUser} /> 
                ...
                <div style={{ textAlign: 'center', width: '100%', padding: '100px 0' }}>
                    <p style={{ color: '#666', fontSize: '15px' }}>Bộ sưu tập đang được cập nhật.</p>
                </div>
            )}
        </div>
    </div>

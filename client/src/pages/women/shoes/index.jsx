...
export default function WomenShoesPage({ setSelectedProduct, addToCart, formatPrice, currentUser, setCurrentUser }) {
    ...
    fetch('http://127.0.0.1:5000/api/products')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const filtered = data.filter(p => p.gender === 'women' && p.category === 'shoes');
          setProductsList(filtered);
        } else {
          const fallback = fallbackProducts.filter(p => p.gender === 'women' && p.category === 'shoes');
